...
TEMP=$(mktemp -u)
nix run "nixpkgs#nvfetcher" -- -c pkgs/apps/packages.toml -o pkgs/apps/_sources -l "${TEMP}"
...
nix run "nixpkgs#nvfetcher" -- -c pkgs/apps/prettier-plugins/plugins.toml -o pkgs/apps/prettier-plugins/_sources
nix run "nixpkgs#nvfetcher" -- -c pkgs/python-modules/packages.toml -o pkgs/python-modules/_sources
